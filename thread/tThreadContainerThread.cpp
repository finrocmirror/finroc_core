  ...
  if (reschedule)
  {
    reschedule = false;
    {
      ...
      while (tasks.Size() > 0)
      {
        ...
        FINROC_LOG_STREAM(rrlib::logging::eLL_WARNING, log_domain, "Detected loop: doing traceback");
        trace_back.Clear();
        ...
        while (true)
        {
          ...
          if (end)
          {
            FINROC_LOG_STREAM(rrlib::logging::eLL_WARNING, log_domain, "Choosing ", current->incoming->GetQualifiedName(), " as next element");
