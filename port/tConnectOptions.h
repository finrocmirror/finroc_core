...
{
  stream << options.conversion_operations << options.flags.Raw();
  return stream;
}

inline rrlib::serialization::tInputStream& operator >> (rrlib::serialization::tInputStream& stream, tConnectOptions& options)
{
  stream >> options.conversion_operations;
  options.flags = tConnectionFlags(stream.ReadInt());
  return stream;
}
