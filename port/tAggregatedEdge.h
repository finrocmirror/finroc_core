...
public:
  ...
  {
    return static_cast<int>(((publish_size.Get() * 1000) / util::tMath::Max(1LL, util::tTime::GetCoarse() - creation_time)));
  }
  ...
  {
    return ((static_cast<float>(publish_count.Get())) * 1000.0f) / (static_cast<float>(util::tMath::Max(1LL, util::tTime::GetCoarse() - creation_time)));
  }
