...
tPort3Method<HANDLER, R, P1, P2, P3>::tPort3Method(tPortInterface& port_interface, const util::tString& name, const util::tString& p1_name, const util::tString& p2_name, const util::tString& p3_name, bool handle_in_extra_thread, int default_net_timeout) :
...
R tPort3Method<HANDLER, R, P1, P2, P3>::Call(tInterfaceClientPort port, tP1Arg p1, tP2Arg p2, tP3Arg p3, int net_timeout)
{
  ...
  else if (ip != NULL && ip->GetType() == tInterfacePort::eServer)
  {
    ...
    R ret = handler->HandleCall(this, p1, p2, p3);
    assert((HasLock(ret)));
    return ret;
  }
  else
...
template<typename HANDLER, typename R, typename P1, typename P2, typename P3>
void tPort3Method<HANDLER, R, P1, P2, P3>::CallAsync(tInterfaceClientPort port, tAsyncReturnHandler<R>* handler, tP1Arg p1, tP2Arg p2, tP3Arg p3, int net_timeout, bool force_same_thread)
{
  //1
  assert((HasLock(p1)));  //2
  assert((HasLock(p2)));  //3
  ...
  else if (ip != NULL && ip->GetType() == tInterfacePort::eServer)
  {
    ...
    if (force_same_thread || (!HandleInExtraThread()))
    {
      try
      {
        R ret = mhandler->HandleCall(this, p1, p2, p3);
        assert((HasLock(ret)));
...
template<typename HANDLER, typename R, typename P1, typename P2, typename P3>
void tPort3Method<HANDLER, R, P1, P2, P3>::ExecuteAsyncNonVoidCallOverTheNet(tMethodCall* mc, tInterfaceNetPort* net_port, tAbstractAsyncReturnHandler* ret_handler, int net_timeout)
{
...
template<typename HANDLER, typename R, typename P1, typename P2, typename P3>
void tPort3Method<HANDLER, R, P1, P2, P3>::ExecuteFromMethodCallObject(tMethodCall* call, tAbstractMethodCallHandler* handler, tAbstractAsyncReturnHandler* ret_handler)
{
...
template<typename HANDLER, typename R, typename P1, typename P2, typename P3>
void tPort3Method<HANDLER, R, P1, P2, P3>::ExecuteFromMethodCallObject(tMethodCall* call, const HANDLER& handler, tAsyncReturnHandler<R>* ret_handler, bool dummy)
{
