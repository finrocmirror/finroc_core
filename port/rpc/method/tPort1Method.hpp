...
tPort1Method<HANDLER, R, P1>::tPort1Method(tPortInterface& port_interface, const util::tString& name, const util::tString& p1_name, bool handle_in_extra_thread, int default_net_timeout) :
...
template<typename HANDLER, typename R, typename P1>
R tPort1Method<HANDLER, R, P1>::Call(tInterfaceClientPort port, tP1Arg p1, int net_timeout)
{
  //1
  assert((HasLock(p1)));
  ...
  else if (ip != NULL && ip->GetType() == tInterfacePort::eServer)
  {
    ...
    R ret = handler->HandleCall(this, p1);
    assert((HasLock(ret)));
    return ret;
  }
  else
...
template<typename HANDLER, typename R, typename P1>
void tPort1Method<HANDLER, R, P1>::CallAsync(const tInterfaceClientPort* port, tAsyncReturnHandler<R>* handler, tP1Arg p1, int net_timeout, bool force_same_thread)
{
  //1
  assert((HasLock(p1)));
  tInterfacePort* ip = port->GetServer();
  ...
  else if (ip != NULL && ip->GetType() == tInterfacePort::eServer)
  {
    ...
    if (force_same_thread || (!HandleInExtraThread()))
    {
      try
      {
        R ret = mhandler->HandleCall(this, p1);
        assert((HasLock(ret)));
...
template<typename HANDLER, typename R, typename P1>
void tPort1Method<HANDLER, R, P1>::ExecuteAsyncNonVoidCallOverTheNet(tMethodCall* mc, tInterfaceNetPort* net_port, tAbstractAsyncReturnHandler* ret_handler, int net_timeout)
{
...
template<typename HANDLER, typename R, typename P1>
void tPort1Method<HANDLER, R, P1>::ExecuteFromMethodCallObject(tMethodCall* call, tAbstractMethodCallHandler* handler, tAbstractAsyncReturnHandler* ret_handler)
{
...
template<typename HANDLER, typename R, typename P1>
void tPort1Method<HANDLER, R, P1>::ExecuteFromMethodCallObject(tMethodCall* call, const HANDLER& handler, tAsyncReturnHandler<R>* ret_handler, bool dummy)
{
  ...
  try
  {
    R ret = handler2->HandleCall(this, p1);
    if (ret_handler != NULL)
    {
      call->Recycle();
